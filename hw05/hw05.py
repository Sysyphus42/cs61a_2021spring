...
def store_digits(n):
    ...
    if res == 0:
        return Link(n)
    return Link(n,store_digits(res))
...
class Mint:
    ...
    def create(self, kind):
        stamp = self.year if self.year else self.current_year
        return kind(stamp)
...
class Coin:
    ...
    def worth(self):
        add = Mint.current_year - self.year - 50
        return self.cents + add if add > 0 else self.cents
...
def is_bst(t):
    ...
    def max_value(t):
        if t.is_leaf():
            return t.label
        return max(t.label, max([max_value(b) for b in t.branches]))
    def min_value(t):
        if t.is_leaf():
            return t.label
        return min(t.label, min([min_value(b) for b in t.branches]))
    if t.is_leaf():
        return True
    if len(t.branches) > 2:
        return False
    ...
    if len(t.branches) == 2:
        if not is_bst(t.branches[0]) or not is_bst(t.branches[1]):
            return False
        if max_value(t.branches[0]) > t.label or min_value(t.branches[1]) <= t.label:
            return False
        return True
