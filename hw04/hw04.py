...
def balanced(m):
    ...
    True
    """
    #The torque applied by its left arm is equal to that applied by its right arm
    def torque(m):
        if is_planet(m):
            return False
        else:
            return length(left(m)) * total_weight(end(left(m))) - length(right(m)) * total_weight(end(right(m)))
    return torque(m) == 0 and torque(end(left(m))) == 0 and torque(end(right(m))) == 0
...
def quadratic(x, a, b, c):
    ...
    return interval(min(f(lbx),f(ubx)),max(f(lbx),f(ubx)))
