...
def about(topic):
    ...
    # BEGIN PROBLEM 2
    "*** YOUR CODE HERE ***"
    return lambda str: any([lower(remove_punctuation(x)) == lower(word) for x in split(str) for word in topic])
    # END PROBLEM 2
...
def wpm(typed, elapsed):
    ...
    # BEGIN PROBLEM 4
    "*** YOUR CODE HERE ***"
    return ((len(list(typed)) / 5) / elapsed) * 60.0
    # END PROBLEM 4
...
def autocorrect(typed_word, valid_words, diff_function, limit):
    ...
    if typed_word in valid_words:
        ...
        a = min(valid_words, key=lambda x: diff_function(typed_word, x, limit))
        return a if diff_function(typed_word, a, limit) <= limit else typed_word
